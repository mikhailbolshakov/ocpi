...
type GeoLocation struct {
	Latitude  string `json:"latitude,omitempty"`  // Latitude of the location
	Longitude string `json:"longitude,omitempty"` // Longitude of the location
}

type AdditionalGeoLocation struct {
	Latitude  string       `json:"latitude,omitempty"`  // Latitude of the location
	Longitude string       `json:"longitude,omitempty"` // Longitude of the location
	Name      *DisplayText `json:"name,omitempty"`      // Name of the point in local language
}

type EnergySource struct {
	Source     string `json:"source,omitempty"`     // Source type of energy source
	Percentage int    `json:"percentage,omitempty"` // Percentage of this source (0-100) in the mix
}

type EnvironmentalImpact struct {
	Category string  `json:"category,omitempty"` // Category environmental impact category
	Amount   float64 `json:"amount,omitempty"`   // Amount of this portion in g/kWh
}

type EnergyMix struct {
	IsGreenEnergy     bool                   `json:"isGreenEnergy,omitempty"`     // IsGreenEnergy true if 100% from regenerative sources
...
type StatusSchedule struct {
	PeriodBegin time.Time  `json:"periodBegin,omitempty"` // PeriodBegin begin of scheduled period
	PeriodEnd   *time.Time `json:"periodEnd,omitempty"`   // PeriodEnd end of schedule period
	Status      string     `json:"status,omitempty"`      // Status value during the scheduled period
}

type RegularHours struct {
	Weekday     int    `json:"weekday,omitempty"`     // Weekday from Monday (1) till Sunday (7)
	PeriodBegin string `json:"periodBegin,omitempty"` // PeriodBegin in 24h format with leading zeros. Example: "18:15"
	PeriodEnd   string `json:"periodEnd,omitempty"`   // PeriodEnd in 24h format with leading zeros. Example: "18:15"
}

type ExceptionalPeriod struct {
	PeriodBegin time.Time `json:"periodBegin,omitempty"` // PeriodBegin begin of the exception. In UTC, time_zone field can be used to convert to local time
	PeriodEnd   time.Time `json:"periodEnd,omitempty"`   // PeriodEnd end of the exception. In UTC, time_zone field can be used to convert to local time
}

type Hours struct {
	TwentyFourSeven     bool                 `json:"twentyFourSeven,omitempty"`     // TwentyFourSeven true to represent 24 hours a day and 7 days a week
...
type Connector struct {
	Id                 string    `json:"id,omitempty"`                 // Id identifier of the Connector
	LocationId         string    `json:"locationId,omitempty"`         // LocationId link to location id
	...
	Standard           string    `json:"standard,omitempty"`           // Standard of the installed connector
	Format             string    `json:"format,omitempty"`             // Format socket/cable
	PowerType          string    `json:"powerType,omitempty"`          // PowerType
	MaxVoltage         float64   `json:"maxVoltage,omitempty"`         // MaxVoltage maximum voltage in V
	MaxAmperage        float64   `json:"maxAmperage,omitempty"`        // MaxAmperage maximum amperage in A
...
type Evse struct {
	Id                  string            `json:"id,omitempty"`                  // Id identifies the EVSE within the CPOs platform
	LocationId          string            `json:"locationId,omitempty"`          // LocationId link to location id
	Status              string            `json:"status,omitempty"`              // Status current status of the EVSE
	EvseId              string            `json:"evseId,omitempty"`              // EvseId following specification for EVSE ID from "eMI3 standard version V1.0". Can be reused
...
type Location struct {
	Id                 string                   `json:"id,omitempty"`                 // Id uniquely identifies the location within the CPOs platform
	Publish            *bool                    `json:"publish,omitempty"`            // Publish if a Location may be published
	...
	Address            string                   `json:"address,omitempty"`            // Address street/block name and house number
	City               string                   `json:"city,omitempty"`               // City or town
	...
	Country            string                   `json:"country,omitempty"`            // Country alpha-3 code for the country
	Coordinates        GeoLocation              `json:"coordinates,omitempty"`        // Coordinates of the location
	...
	TimeZone           string                   `json:"timeZone,omitempty"`           // TimeZone IANA tzdata’s TZ-values
...
type EvseSearchResponse struct {
	PageInfo *PageResponse `json:"pageInfo,omitempty"`
	Items    []*Evse       `json:"items,omitempty"`
}
